    ...
    switch (letra) {
      case "q":
        ...
        if (i + 2 < nameLength) {
          ...
          do {
            letra = consonantes[Math.floor(Math.random() * cantCons)];
          } while (letra === "q");
        }
        break;
...
function generarPropuesta() {
  ...
  for (let i = 0; i < propuestas.length; i++) {
    const tipoDePropuesta = propuestas[i]; // objeto
    const parteDelaPropuesta = Object.keys(tipoDePropuesta); // [ intenciones, acciones, etc ]

    for (let j = 0; j < parteDelaPropuesta.length; j++) {
      let posiblesParteDelTexto = tipoDePropuesta[parteDelaPropuesta[j]]; // ["Queremos", "Pensamos","Planeamos", etc]
      let randomIndex = Math.floor(
        Math.random() * posiblesParteDelTexto.length
      );
      let texto = posiblesParteDelTexto[randomIndex];
      frase += texto;
    }
    frase += "<br><br>";
  }
...
function votar() {
  ...
  if (candidatos.length < 2)
    return (msg.innerHTML = "Se necesitan al menos 2 candidatos para votar");
  ...
  if (numDesempatantes >= 2) {
    ...
    desempatantes.forEach((desempatante) => {
      ...
      anuncioEmpate += `<br> - ${desempatante.nombre} ${desempatante.apellido} (${desempatante.ideologia})`;
      // "<br>" +
      // "- " +
      // desempatante.nombre +
      // " " +
